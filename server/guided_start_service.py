...
def load_guided_start_catalog() -> GuidedStartCatalog:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Guided start catalog could not be read.") from exc
    except JSONDecodeError as exc:
        raise HTTPException(status_code=500, detail=f"Guided start catalog is invalid: {exc.msg}") from exc
    except ValidationError as exc:
        raise HTTPException(status_code=500, detail="Guided start catalog failed validation.") from exc
