...
async def analyze_trace_endpoint(trace_id: str, graph_id: str | None = None) -> dict:
    ...
    # resolve graph
    if graph_id:
        if trace.graph_id and trace.graph_id != graph_id:
            raise HTTPException(
                status_code=400,
                detail="trace and graph_id do not match",
            )
        graph = db_get_graph(graph_id)
        if not graph:
            raise HTTPException(status_code=404, detail=f"Graph not found: {graph_id}")
    elif trace.graph_id:
        graph = db_get_graph(trace.graph_id)
        if not graph:
            raise HTTPException(
                status_code=400,
                detail=f"Trace is associated with missing graph: {trace.graph_id}",
            )
    else:
        ...
        if len(graphs) == 1:
            ...
            raise HTTPException(
                status_code=400,
                detail="Trace is not associated with a graph. Provide graph_id.",
            )
        else:
...
def get_trace_cognition(trace_id: str, graph_id: str | None = None) -> dict:
    """Get cached cognition results for a trace."""
    from server.cognition_db import delete_cognition, get_cognition

    trace = get_trace(trace_id)
    if not trace:
        raise HTTPException(status_code=404, detail=f"Trace not found: {trace_id}")

    if graph_id and trace.graph_id and graph_id != trace.graph_id:
        raise HTTPException(status_code=400, detail="trace and graph_id do not match")
    ...
    expected_graph_id = graph_id or trace.graph_id
    if expected_graph_id and cognition.graph_id != expected_graph_id:
        delete_cognition(trace_id)
        raise HTTPException(status_code=404, detail="Cached cognition is stale. Run POST /analyze again.")
