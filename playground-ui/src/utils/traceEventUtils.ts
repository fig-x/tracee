...
export function diffStateKeys(
  ...
  return Array.from(new Set([...Object.keys(previousState), ...Object.keys(nextState)]))
    .filter((key) => {
      const hasPreviousKey = Object.prototype.hasOwnProperty.call(previousState, key);
      const hasNextKey = Object.prototype.hasOwnProperty.call(nextState, key);
      const previousValue = previousState[key];
      const nextValue = nextState[key];
      if (
        hasPreviousKey
        && hasNextKey
        && isEmptyStateValue(previousValue)
        && isEmptyStateValue(nextValue)
      ) {
        return false;
      }
