...
describe('analyzeRunsWithReference', () => {
  ...
  it('reuses the existing run point when a matching run is promoted as anchor', () => {
    ...
    const entries = buildVisualizationEntries(analysis, {
      ...
      source: 'run',
      runIndex: 1,
      selectionId: 'primary:1',
    });

    expect(entries).toHaveLength(2);
    const anchorEntries = entries.filter((entry) => entry.kind === 'anchor');
    expect(anchorEntries).toHaveLength(1);
    expect(anchorEntries[0].selectionId).toBe('primary:1');
  });
  ...
  it('keeps anchor similarity metrics when a promoted run reuses its point', () => {
    ...
    expect(anchorSimilarityMap.has('primary:1')).toBe(false);
