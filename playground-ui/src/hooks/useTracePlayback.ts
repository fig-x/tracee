...
export function computeExecutionFrames(
  ...
  const rootStartsByRunId = new Map<string, TraceEvent>();
  for (const event of orderedEvents) {
    if (event.event_type !== "on_chain_start" || !isRecord(event.payload?.inputs)) continue;
    const nodeId = resolveAgentNodeId(event, nodeIdSet, runToNode, spanToNode);
    const runId = getRunIdFromEvent(event);
    if (!nodeId || !runId) continue;
    if (!rootRunIdsByNode.get(nodeId)?.has(runId)) continue;
    rootStartsByRunId.set(runId, event);
  }
  ...
  orderedEvents.forEach((event, eventOrder) => {
    ...
    const startEvent = rootStartsByRunId.get(runId);
    const changedKeys = isRecord(startEvent?.payload?.inputs)
      ? diffStateKeys(startEvent.payload.inputs, nextState)
      : [];
