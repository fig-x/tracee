...
const EMPTY_OPERATIONS: AgentOperation[] = [];
...
export function CognitionDetails({ node }: Props) {
  ...
  const showUnmatchedChip = (chipValue: string) => {
    if (unmatchedTimer.current != null) clearTimeout(unmatchedTimer.current);
      ...
      <section className="side-panel__section">
        ...
        <div className="side-panel__card">
          <p className="side-panel__empty">
            no cognition analysis available yet. click the <strong>analyze</strong> button on the canvas to generate insights.
          </p>
        </div>
      </section>
    );
  ...
  return (
    <>
      ...
      {expandedOp && (
        <section className="side-panel__section">
          <h3 className="side-panel__section-title cognition-detail__op-header">
            <span className="cognition-detail__op-title">
              <span
                className="cognition-detail__op-icon"
                style={getMaskIconStyle(operationIconMap[expandedOp.type])}
                aria-hidden
              />
              <span className="cognition-detail__op-label">{expandedOp.label}</span>
            </span>
            <button
