...
import { useSidebar, type ChipExpansion } from "../../context/SidebarContext";
...
export function GraphViewer() {
  ...
  const {
    ...
    analyze: runAnalysis,
  } = useCognitionOverlay(selectedTraceId, graphId, displayNodes, playbackEdges, isCognitionLayer);
  ...
  const focusAndOpenNode = useCallback((
    ...
    const node = mergedNodes.find((candidate) => candidate.id === nodeId);
    if (!node) return;
    focusNodeInCanvas(nodeId);
    openSidebar(nodeId, node.data, expansion, operationId ?? null);
  }, [mergedNodes, focusNodeInCanvas, openSidebar]);

  const handleTraceOutlineSelect = useCallback((item: TraceOutlineItem) => {
    if (!item.nodeId) return;
    focusAndOpenNode(item.nodeId, undefined, item.operationId ?? null);
  }, [focusAndOpenNode]);
