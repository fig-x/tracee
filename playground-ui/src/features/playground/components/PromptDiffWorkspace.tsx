...
interface CurrentDraft {
  /** prompt id + version id for the identity row */
  identityLabel: string;
  /** commit / update / version title (optional second line) */
...
const PromptDiffWorkspace: React.FC<Props> = ({
  ...
  const compareIdentityLabel = `${target.promptId} / ${target.version.version_id}`;
  const compareMessageLabel =
    target.version.revision_note?.trim()
    || target.version.name?.trim()
    || null;
  const currentMessageLabel =
    currentDraft.messageLabel?.trim()
    || currentDraft.revisionNote?.trim()
    || null;
      ...
      <div className="version-compare__meta-grid">
        <div className="card">
          <div className="card__body version-compare__meta-card">
            ...
            <div className="version-compare__version-name">{currentDraft.identityLabel}</div>
            {currentMessageLabel ? (
              <div className="field__hint">{currentMessageLabel}</div>
            ) : null}
        ...
        <div className="card version-compare__meta-card-shell version-compare__meta-card-shell--compare">
          <div className="card__body version-compare__meta-card version-compare__meta-card--compare">
            ...
            <div className="version-compare__version-name">{compareIdentityLabel}</div>
            {compareMessageLabel ? (
              <div className="field__hint">{compareMessageLabel}</div>
            ) : null}
            <div className="version-compare__stat-row">
