...
import {
  getGuidedTemplateComponentOrder,
...
const PromptForm: React.FC<Props> = ({
  ...
  const [guidedAnalysisPromptCount, setGuidedAnalysisPromptCount] = React.useState<number | null>(null);
                  ...
                  <div className="create-run__workspace">
                    ...
                    <div className="create-run__workspace-body" ref={workspaceBodyRef}>
                      ...
                      {editorView === 'diff' && editorCompareTarget && (
                        <PromptDiffWorkspace
                          currentDraft={{
                            identityLabel:
                              currentEditorPromptId && currentEditorVersionId
                                ? `${currentEditorPromptId} / ${currentEditorVersionId}`
                                : currentEditorVersionId
                                  ? currentEditorVersionId
                                  : currentEditorPromptId
                                    ? `${currentEditorPromptId} · draft`
                                    : currentEditorVersionLabel,
                            messageLabel:
                              revisionNote.trim()
                              || loadedPromptContext?.version.revision_note?.trim()
                              || loadedPromptContext?.version.name?.trim()
                              || null,
                    ...
                    {guidedOverlayStep && selectedGuidedRole && (
                      <GuidedOverlay
                        ...
                        analysisPromptCount={guidedAnalysisPromptCount}
                        getAnchor={() => (
                    ...
                    {activePanel === 'guided' && (
                      <div className="create-run__panel-surface">
                        <GuidedPromptStart
                          ...
                          onSelectRole={(role, analysisPromptCount) => {
                            const orderedRole = {
                              ...role,
                              components: [...role.components].sort(
                                (left, right) =>
                                  getGuidedTemplateComponentOrder(left.component_type)
                                  - getGuidedTemplateComponentOrder(right.component_type)
                              ),
                            };
                            setSelectedGuidedRole(orderedRole);
                            setGuidedAnalysisPromptCount(analysisPromptCount);

                            const components = orderedRole.components.map((comp) => ({
                            ...
                            setAppliedTemplateId(`guided-${orderedRole.role_id}`);
                            setPromptNameInput((current) => current || `${orderedRole.name} prompt`);
                            setCollapsedSections({});
                            setPendingScrollSectionKey(null);
                            setEditorCompareTarget(null);
                            setEditorView('components');
